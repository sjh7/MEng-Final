...
#_________________Navigation Algorithm_____________________
  ...
  #________Battery Life Management______________
	...
	self.take_off_connections = 0
        ...
        if self.Battery_life <= 99 and self.Battery_life >= 98.01:  #and self.turnedBack == False:
            print "Battery Low 40% - Drone landing, manual relaunch available"
            #self.battery_has_control = 1
	    self.e_land = 0
	    self.land()
	    
	if self.Battery_life <= 98: #and self.turnedBack == False:
            print "Battery Too Low (Below 50%) - DRONE LANDING! CRITICAL BATTERY! manual relaunch 		    unavailable "
            #self.battery_has_control = 1
	    self.e_land = 1
	    if self.e_land == 1:
			self.land()
			rospy.signal_shutdown('Low Battery!')
